...
import { processVideoPromptPipeline, parseVideoPrompt, buildVideoPrompt, validateVideoPrompt, generateDynamicNegativePrompt } from '../utils/promptValidator';
...
export const GeneratorDashboard: React.FC<GeneratorDashboardProps> = ({
  ...
  const promptAnalysis = processVideoPromptPipeline(prompt, negativePrompt, style, cameraControl);
  ...
  const handleGenerate = async () => {
    ...
    const pipelineResult = processVideoPromptPipeline(prompt, negativePrompt, style, cameraControl);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="rounded-2xl p-5 glass-panel border-slate-800 shadow-xl space-y-4">
            ...
            {/* Prompt Analysis & Validation Inspector */}
            <div className="pt-1">
              <button
                type="button"
              ...
              >
                ...
                <span>{showPromptInspector ? 'Hide Strict Prompt Analysis' : 'Inspect Strict Prompt Following & Validation'}</span>
              </button>

              {showPromptInspector && (
                <div className="mt-3 p-4 rounded-xl bg-cyan-950/20 border border-cyan-800/40 space-y-3 animate-in fade-in text-xs text-slate-300">
                  <div className="flex items-center justify-between border-b border-cyan-800/30 pb-2">
                    <span className="font-bold text-cyan-300 flex items-center gap-1.5">
                      <Check className="w-3.5 h-3.5 text-emerald-400" />
                      Strict Prompt Validation Check
                    </span>
                    <span className="px-2 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-800/60 font-mono text-[10px]">
                      PASSED ✓
                    </span>
                  </div>
                  <p className="text-slate-400 italic">{promptAnalysis.validationLog}</p>
                  
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1 font-mono text-[11px]">
                    <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                      <span className="text-slate-500 block">Subject:</span>
                      <span className="text-white font-semibold">{promptAnalysis.parsed.subject || 'N/A'}</span>
                    </div>
                    <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                      <span className="text-slate-500 block">Action:</span>
                      <span className="text-white font-semibold">{promptAnalysis.parsed.action || 'N/A'}</span>
                    </div>
                    <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                      <span className="text-slate-500 block">Location:</span>
                      <span className="text-white font-semibold">{promptAnalysis.parsed.location || 'N/A'}</span>
                    </div>
                    <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                      <span className="text-slate-500 block">Lighting:</span>
                      <span className="text-white font-semibold">{promptAnalysis.parsed.lighting || 'N/A'}</span>
                    </div>
                    <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                      <span className="text-slate-500 block">Weather:</span>
                      <span className="text-white font-semibold">{promptAnalysis.parsed.weather || 'N/A'}</span>
                    </div>
                    <div className="bg-slate-900/80 p-2 rounded border border-slate-800">
                      <span className="text-slate-500 block">Appearance:</span>
                      <span className="text-white font-semibold">{promptAnalysis.parsed.appearance || 'N/A'}</span>
                    </div>
                  </div>

                  <div>
                    <span className="text-slate-500 block mb-1">Generated Safety Negative Prompt (Excluding Drift):</span>
                    <div className="bg-slate-950 p-2.5 rounded border border-slate-800 text-[11px] font-mono text-rose-300">
